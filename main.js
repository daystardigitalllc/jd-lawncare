document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Before & After Universal Layered Slider Handler ---
  const initBeforeAfterSliders = () => {
    ...
    containers.forEach(container => {
      ...
      window.addEventListener('touchmove', (e) => {
        if (isDragging && e.touches.length > 0) updatePos(e.touches[0].clientX);
      }, { passive: true });
    ...
    // Preset Tabs Switcher for Featured Slider
    const baTabs = document.querySelectorAll('.ba-tab-btn');
    const mainContainer = document.getElementById('ba-slider');
